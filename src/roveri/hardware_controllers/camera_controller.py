...
from roveri import tf_models
from roveri.constants import object_detection_values
from roveri.hardware_controllers.singleton import Singleton
from roveri.image_processing.tf_object_detection import get_detections
...
class CameraController(metaclass=Singleton):
    ...
    def setup(self) -> None:
        ...
        base_options = core.BaseOptions(
            file_name=self._model,
            use_coral=self._enable_edgetpu,
            num_threads=self._number_threads,
        )
        detection_options = processor.DetectionOptions(
            max_results=object_detection_values.MAX_RESULTS,
            score_threshold=object_detection_values.SCORE_THRESHOLD,
        )
...
class CameraControllerRaspberry(CameraController):
    ...
    def setup(self) -> None:
        ...
        self._picamera.preview_configuration.main.size = (
            self._camera_width,
            self._camera_height,
        )
        ...
        self._picamera.configure(
            "preview"
        )  # TODO: titulo de la ventana: probablemente no se ocupe dado
