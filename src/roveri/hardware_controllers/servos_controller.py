...
class ServosControllerMock(ServosController):
    ...
    def toggle(self, servo_pair: ServoPair) -> None:
        ...
        logger.info(
            f"moving to {self._servos_values[servo_pair][self._servos_status[servo_pair]]}"
        )
        logger.info(
    ...
    def move(
        self, servo_pair: ServoPair, status: ServoStatus, bypass_check: bool = False
    ) -> None:
...
class ServosControllerRaspberry(ServosController):
    ...
    def _get_servos(self, servo_pair: ServoPair) -> tuple[servo.Servo, servo.Servo]:
        if self._pca is None:
            raise RuntimeError(
                "ServosControllerRaspberry.setup() must be called before using the servos"
            )
