...
try:
    ...
    logger.warning(
        "Could not import busio, board, adafruit_ssd1306. This is expected when running on a computer"
    )
...
class OLEDDisplayControllerRaspberry(OLEDDisplayController):
    ...
    def _display(self) -> None:
        ...
        self._oled_display.text(
            self._display_message.state, 0, 0, 1, font_name=self._font
        )
        self._oled_display.text(
            self._display_message.substate, 0, 8, 1, font_name=self._font
        )
        self._oled_display.text(
            self._display_message.message, 0, 16, 1, font_name=self._font
        )
        self._oled_display.text(
            self._display_message.debug, 0, 24, 1, font_name=self._font
        )
