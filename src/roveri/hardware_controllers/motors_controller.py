...
from roveri.hardware_controllers.mockGPIO import MockGPIO
...
class MotorsControllers(metaclass=Singleton):
    ...
    @abstractmethod
    def move(
        self, motor_side: MotorSide, speed: int, direction: MotorDirection
    ) -> None:
        pass
...
class MotorsControllerMock(MotorsControllers):
    ...
    def move(
        ...
        logger.info(
            f"Moving {motor_side.name} motors with speed: {speed} and direction {direction.name}"
        )
