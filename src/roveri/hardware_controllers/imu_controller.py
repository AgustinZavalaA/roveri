...
def gyroscope_any_iqr_strategy(
    full_data: Mapping[DataRecollectedType, npt.NDArray[np.float64]]
) -> bool:
...
def gyroscope_all_iqr_strategy(
    full_data: Mapping[DataRecollectedType, npt.NDArray[np.float64]]
) -> bool:
    data = full_data[DataRecollectedType.GYROSCOPE]
    q1, q3 = np.percentile(data, [25, 75], axis=0)
...
def gyroscope_all_std_strategy(
    full_data: Mapping[DataRecollectedType, npt.NDArray[np.float64]]
) -> bool:
    data = full_data[DataRecollectedType.GYROSCOPE]
...
def accelerometer_all_std_strategy(
    full_data: Mapping[DataRecollectedType, npt.NDArray[np.float64]]
) -> bool:
    data = full_data[DataRecollectedType.ACCELEROMETER]
    accel_std = np.std(data, axis=0)
    return bool(
        np.all(accel_std < 0.02)
    )  # TODO: use a config file to set the threshold


def accelerometer_all_iqr_strategy(
    full_data: Mapping[DataRecollectedType, npt.NDArray[np.float64]]
) -> bool:
    data = full_data[DataRecollectedType.ACCELEROMETER]
    q1, q3 = np.percentile(data, [25, 75], axis=0)
...
class IMUController(metaclass=Singleton):
    ...
    def is_robot_stuck(
        self,
        strategy: Callable[
            [Mapping[DataRecollectedType, npt.NDArray[np.float64]]], bool
        ],
    ) -> bool:
        pass
...
class IMUControllerMock(IMUController):
    ...
    def is_robot_stuck(
        ...
        logger.info(
            "IMUControllerMock.is_robot_stuck() called with strategy: " + str(strategy)
        )
        return False
...
class IMUControllerMockRaspberry(IMUController):
    ...
    def is_robot_stuck(
        ...
        self._data_index = (
            self._data_index + 1
        ) % NUM_SAMPLES  # TODO: use a config file to set the size of the array
...
def main():
    ...
    try:
        while True:
            ...
            print(
                f"stuck all iqr accel: {imu_controller.is_robot_stuck(accelerometer_all_iqr_strategy)}"
            )  # best accel
