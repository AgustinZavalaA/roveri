...
try:
    ...
    logging.getLogger(__name__).warning(
        "RPi.GPIO not found, using mock buzzer controller"
    )
...
class BuzzerController(metaclass=Singleton):
    ...
    def play(self, melody: Melody) -> None:
        ...
        """
        threading.Thread(
            target=self._background_play, args=(melody,), daemon=True
        ).start()
...
class BuzzerControllerMock(BuzzerController):
    ...
    def _background_play(self, melody: Melody) -> None:
        ...
        for note in self._melodies[melody]:
            logger.info(
                f"Playing a tone with frequency {note.frequency} and duration {note.duration}"
            )
